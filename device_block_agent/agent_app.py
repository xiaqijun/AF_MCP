...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "AF 可复用默认登录配置与登录态；USG 使用 Basic Auth，不维护独立登录会话，通常应先通过聊天工具 set_usg_connection 录入连接账号。"
        "若登录态缺失、配置不完整、目标设备不明确或确认条件不足，要先指出阻塞点，再给出最小下一步。"
...
def addition_system_instruction() -> str:
    return (
        ...
        "AF 可复用登录态；USG 不维护独立登录会话，应优先通过 set_usg_connection 在聊天中录入连接账号。"
        "若登录态缺失、配置不完整、目标不明确或确认条件不足，要直接指出阻塞点。"
